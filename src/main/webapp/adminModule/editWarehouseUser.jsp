...
<html>
<head>
    <title>Editar usuario de bodega</title>
...
<section class="section pb-0">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h2 class="section-title text-primary">Crear o editar usuarios de bodega</h2>
                ...
                <div id="accordion">
                        ...
                        <div id="create-user" class="collapse" data-parent="#accordion">
                            <div class="card-body font-secondary text-color">
                                <form action="${pageContext.request.contextPath}/manage-user-servlet" method="POST">
                                    ...
                                    <input type="hidden" id="newUserType" name="newUserType" value="warehouse_user">
                                    <!-- storeId input -->
                                    <input type="text" name="newStoreId" id="newStoreId" placeholder="Id de la tienda" class="form-control mb-4 shadow rounded-0"/>
                                    <!-- Name input -->
                        ...
                        <div id="edit-user" class="collapse" data-parent="#accordion">
                            <div class="card-body font-secondary text-color">
                                <table>
                                    ...
                                    <tbody>
                                    <c:forEach items="${warehouseUsers}" var="whUser">
                                        <tr>
                                            <td><strong>${whUser.id}</strong></td>
                                            <td><em>${whUser.name}</em></td>
                                            <td><em>${whUser.username}</em></td>
                                            <td><em>${whUser.email}</em></td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
                                </table>
                                <form action="${pageContext.request.contextPath}/manage-user-servlet" method="POST">
                                    <!-- hidden UserType input -->
                                    <input type="hidden" id="userType" name="userType" value="warehouse_user">
                                    <!-- UserId input -->
                    ...
                    <div class="card mb-4 rounded-0 shadow border-0">
                        <div class="card-header rounded-0 bg-white border p-0 border-0">
                            <a class="card-link h4 d-flex tex-dark mb-0 py-3 px-4 justify-content-between" data-toggle="collapse"
                               href="#edit-stores-assigned">
                                <span>Editar tiendas asignadas</span> <i
                                    class="ti-plus text-primary text-right"></i>
                            </a>
                        </div>
                        <div id="edit-stores-assigned" class="collapse" data-parent="#accordion">
                            <div class="card-body font-secondary text-color">
                                <table>
                                    <thead>
                                    <tr>
                                        ...
                                        <th>ID tienda</th>
                                        <th>ID usuario encargado</th>
                                        <th>Username encargado</th>
                                        <th>Nombre encargado</th>
                                        <th>Email encargado</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <c:set var="whUsersStores" value="${sessionScope.whUsersStores}"/>
                                    <c:forEach var="whUserStoreRow" items="${whUsersStores}">
                                        <tr>
                                            <c:forEach var="whUserStoreCol" items="${whUserStoreRow}" >
                                                <td><em>${whUserStoreCol}</em></td>
                                            </c:forEach>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
                                ...
                                <form action="${pageContext.request.contextPath}/manage-user-servlet" method="POST">
                                    <!-- hidden UserType input -->
                                    <input type="hidden" id="whUserType" name="whUserType" value="warehouse_user">
                                    <!-- storeId input -->
                                    <input type="text" name="whStoreId" id="whStoreId" placeholder="Id de la tienda a asignar" class="form-control mb-4 shadow rounded-0"/>
                                    <!-- UserId input -->
                                    <input type="text" name="whUserId" id="whUserId" placeholder="Id del usuario al que se asignará" class="form-control mb-4 shadow rounded-0"/>
                                    <!-- hidden identifier -->
                                    <input type="hidden" name="updatewhs" value="true">
                                    <!-- Submit button -->
                                    <button type="submit" class="btn btn-primary btn-block mb-4">Modificar</button>
                                </form>
                            </div>
